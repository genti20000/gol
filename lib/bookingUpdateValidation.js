...
const validateBookingUpdateInput = (input) => {
  ...
  if (input.bookingToken !== undefined) {
    if (typeof input.bookingToken !== 'string') {
      fieldErrors.bookingToken = 'bookingToken must be a string.';
    } else {
      normalized.bookingToken = input.bookingToken.trim();
    }
  }

  if (input.token !== undefined) {
    if (typeof input.token !== 'string') {
      fieldErrors.token = 'token must be a string.';
    } else {
      normalized.token = input.token.trim();
    }
  }
