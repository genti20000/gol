            ...
            <div className="flex flex-col text-left gap-2">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const matched = activeServices.find((service) => service.id === nextServiceId);
                  if (matched) setGuests(matched.minPeople);
                }}
                className="bg-zinc-900/50 border-zinc-800 border rounded-xl md:rounded-2xl px-5 py-3.5 md:py-4 focus:ring-1 ring-amber-500 outline-none text-white w-full font-bold appearance-none shadow-inner min-h-[44px]"
              >
                {activeServices.map(service => <option key={service.id} value={service.id} className="bg-zinc-950">{service.name}</option>)}
              </select>
            </div>
