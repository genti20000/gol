...
type ServiceRow = {
  ...
  price_per_person_pence: number | null;
...
const mapBookingMutationError = (error: any): string => {
  const code = typeof error?.code === 'string' ? error.code : '';
  const message = typeof error?.message === 'string' ? error.message : 'Unknown error';

  // Exclusion-constraint overlap (bookings_no_overlap_per_room)
  if (code === '23P01' || message.toLowerCase().includes('bookings_no_overlap_per_room')) {
    return 'This room already has an overlapping booking for that time.';
  }
  // Unique violation (e.g. booking_ref collisions)
  if (code === '23505') {
    return 'Duplicate booking data was detected. Please refresh and try again.';
  }
  // Check-constraint violation
  if (code === '23514') {
    return 'Booking failed validation. Please review the entered booking values.';
  }
...
export function StoreProvider({ children, mode = 'public' }: { children: React.ReactNode; mode?: StoreMode }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (servicesData) setServices((servicesData as ServiceRow[]).map((s, index) => {
        ...
        if (blocksData) setBlocks((blocksData as any[]).map((b: any) => ({
          ...b,
          roomId: b.room_id,
          createdAt: new Date(b.created_at).getTime()
        })));
        if (specialHoursData) setSpecialHours((specialHoursData as any[]).map((sh: any) => ({
          ...sh,
          open: sh.open_time,
          close: sh.close_time
        })));
        if (operatingHoursData) setOperatingHours((operatingHoursData as any[]).map((oh: any) => ({
          day: oh.day,
          open: oh.open_time,
          close: oh.close_time,
          enabled: oh.enabled
        })));
        const resolvedSettings = (settingsData ?? null) as any;
        setSettings({
          ...DEFAULT_SETTINGS,
        ...
        if (extrasData) setExtras((extrasData as any[]).map((e: any) => ({
          ...e,
          infoText: e.info_text ?? null,
          pricingMode: e.pricing_mode as 'flat' | 'per_person',
          sortOrder: e.sort_order
        })));
        if (staffData) setStaff((staffData as any[]).map((s: any) => ({
          ...s,
          servicesOffered: s.services_offered,
          bufferBefore: s.buffer_before,
          bufferAfter: s.buffer_after
        })));
        if (recurringBlocksData) setRecurringBlocks((recurringBlocksData as any[]).map((rb: any) => ({
          ...rb,
        ...
        if (promoCodesData) setPromoCodes((promoCodesData as any[]).map((pc: any) => ({
          ...pc,
        ...
        if (waitlistData) setWaitlist((waitlistData as any[]).map((w: any) => ({
          ...w,
  ...
  const addBooking = useCallback(async (booking: Partial<Booking>) => {
    ...
    if (!resolvedRoomName && normalizedRoomId) {
      ...
      resolvedRoomName = (roomData as any)?.name?.trim() ?? '';
    }
    ...
    if (error) {
      const mappedError = mapBookingMutationError(error);
      const message = mappedError ? `Database Error: ${mappedError}` : 'Database Error: Unable to create booking.';
      console.error('BOOKING_CONFIRM_ERROR', {
        error: { message: error.message, hint: (error as any).hint, code: (error as any).code, mappedError },
  ...
  const updateBooking = useCallback(async (id: string, patch: Partial<Booking>): Promise<MutationResult> => {
    ...
    if (error) {
      const mappedError = mapBookingMutationError(error);
      console.error('Failed to update booking.', { message: error.message, hint: (error as any).hint, code: (error as any).code, mappedError, payloadKeys: Object.keys(payload), booking_date: payload.booking_date });
