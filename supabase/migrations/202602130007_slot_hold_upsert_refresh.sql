-- Ensure hold refresh for the same session+slot is conflict-free and idempotent.
CREATE UNIQUE INDEX IF NOT EXISTS slot_holds_session_slot_unique
  ON public.slot_holds (service_id, date, start_time, session_id);
...
BEGIN
  ...
  ON CONFLICT (service_id, date, start_time, session_id)
  DO UPDATE SET
    expires_at = EXCLUDED.expires_at,
    created_at = now();

  -- Keep only one active hold per session to avoid stale self-held slots.
  DELETE FROM public.slot_holds
